...
def _ensure_torch():
    global _torch, _nn, _Rearrange
    if _torch is not None and _nn is not None and _Rearrange is not None:
        return _torch, _nn, _Rearrange
    else:
        import torch
...
class PhAI():
  def __init__(self):
    global PhAI_instance
    self.model = None
    self.torch = None
    self.nn = None
    self.Rearrange = None
    self.model_args = {}
    if PhAI_instance is None:
      PhAI_instance = self
    else:
      raise Exception("Only one instance of PhAI class is allowed!")

  def load_model(self, model_path=model_file):
    #check if we have torch and einops installed
    global _torch, _nn, _Rearrange
    try:
      self.torch, self.nn, self.Rearrange = _ensure_torch()
    ...
    class ConvolutionalBlock(self.nn.Module):
      def __init__(self, filters, kernel_size, padding):
          super().__init__()  

          self.act = self.nn.GELU()  

          self.conv1 = self.nn.Conv3d(filters, filters, kernel_size = kernel_size, padding = padding)
          self.conv2 = self.nn.Conv3d(filters, filters, kernel_size = kernel_size, padding = padding)  

          self.norm1 = self.nn.GroupNorm(filters, filters)
          self.norm2 = self.nn.GroupNorm(filters, filters)  
    ...
    class MLPLayer(self.nn.Module):
      def __init__(self, token_nr, dim, dim_exp, mix_type):
          super().__init__()  

          self.act    = self.nn.GELU()  

          self.norm1  = self.nn.GroupNorm(token_nr, token_nr)  

          if mix_type == 'token':
              self.layer1 = self.nn.Conv1d(kernel_size = 1, in_channels = token_nr, out_channels = dim_exp)
              self.layer2 = self.nn.Conv1d(kernel_size = 1, in_channels = dim_exp, out_channels = token_nr)
          else:
              self.layer1 = self.nn.Linear(dim , dim_exp)
              self.layer2 = self.nn.Linear(dim_exp, dim)
    ...
    class PhAINeuralNetwork(self.nn.Module):
      def __init__(self, *, max_index, filters, kernel_size, cnn_depth, dim, dim_exp, dim_token_exp, mlp_depth, reflections):
          ...
          self.net_a = self.nn.Sequential(
              self.Rearrange('b x y z  -> b 1 x y z '),  

              self.nn.Conv3d(1, filters, kernel_size = kernel_size, padding=padding),
              self.nn.GELU(),
              self.nn.GroupNorm(filters, filters)
          )  

          self.net_p = self.nn.Sequential(
              self.Rearrange('b x y z  -> b 1 x y z '),  
          ...
          self.net_convolution_layers = self.nn.Sequential(
              *[self.nn.Sequential(
                  ConvolutionalBlock(filters, kernel_size = kernel_size, padding = padding),
              ) for _ in range(cnn_depth)],
          )  

          self.net_projection_layer = self.nn.Sequential(
              self.Rearrange('b c x y z  -> b c (x y z)'),
              self.nn.Linear(hkl[0]*hkl[1]*hkl[2], dim),
          )  

          self.net_mixer_layers = self.nn.Sequential(
              *[self.nn.Sequential(
                  MLPLayer(mlp_token_nr, dim, dim_token_exp, 'token'),
                  MLPLayer(mlp_token_nr, dim, dim_exp      , 'channel'),
              ) for _ in range(mlp_depth)],
              self.nn.LayerNorm(dim),
          )  

          self.net_output = self.nn.Sequential(
              self.Rearrange('b t x -> b x t'),
              self.nn.AdaptiveAvgPool1d(1),
              self.Rearrange('b x 1 -> b x'),

              self.nn.Linear(dim, reflections*2),
              self.Rearrange('b (c h) -> b c h ', h = reflections),
          )  
    ...
    self.model = PhAINeuralNetwork(**self.model_args)
    state = self.torch.load(model_path, weights_only = True)
    self.model.load_state_dict(state)
  
  def randomize_output(self, output):
      shape = output[0].shape
      rand_mask = self.torch.randint(0, 2, shape)
      output[0][rand_mask == 1] = -180.
      output[0][rand_mask == 0] = 0.
      return output
  
  def phases(self, output_phases):
      bin_size = 180.0
      offset   = bin_size / 2
      #bin_nr   = int(360 / bin_size)
      output_phases = output_phases.permute(0,2,1)
      output_phases = self.torch.argmax(output_phases, dim=2)
      return offset + (output_phases*bin_size) - 180.00 - (bin_size/2)
  
  def reindex_monoclinic(self, H):
      #to: (-h, h), (0, k), (0, l)
  ...
  def prepare_reflections(self, H, F):
      for i in range(len(F)):
        F[i] = F[i]/np.max(F)
      H_reind = self.reindex_monoclinic(H)
  ...
  def get_PhAI_phases(self, f_sq_obs, randomize_phases = 0, cycles = 1):
      """
      ...
      max_index = self.model_args['max_index']
      ...
      indices, Fs = self.prepare_reflections(indices, Fs)
  
      amplitudes = self.torch.zeros(1, 21, 11, 11)
      ...
      if randomize_phases :
        init_phases = randomize_output(self.torch.zeros_like(amplitudes))
      else:
        init_phases = self.torch.zeros_like(amplitudes)
      
      for i in range(cycles):
        print('cycle: ', i+1)
        if i == 0:
          output = self.phases(self.model(amplitudes, init_phases))
        else:
  ...
  def create_solution_map(self, cycles = 1, resolution = 0.2, max_peaks=15):
    cctbx_adapter = OlexCctbxAdapter()
    f_sq_obs = cctbx_adapter.reflections.f_sq_obs_merged
    guess = self.get_PhAI_phases(f_sq_obs, randomize_phases=1, cycles=int(cycles))
...
PhAI_instance = PhAI()
OV.registerFunction(PhAI_instance.create_solution_map, False, 'PhAI')
