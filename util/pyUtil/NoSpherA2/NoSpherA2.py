...
class NoSpherA2(PT):
    def __init__(self):
        ...
        if os.path.exists(self.NoSpherA2):
            self.basis_dir = os.path.join(
                os.path.split(self.NoSpherA2)[0], "basis_sets"
            ).replace("\\", "/")
    ...
    def set_f_calc_obs_sq_one_h_linearisation(
        self, f_calc, f_obs_sq, one_h_linarization
    ):
        self.f_calc = f_calc
        self.f_obs_sq = f_obs_sq
        self.one_h_linearisation = one_h_linarization
    ...
    def setup_har_executables(self):
        ...
        if sys.platform[:3] != "win":
            ...
            if "LD_LIBRARY_PATH" in os.environ:
                ...
                os.environ["LD_LIBRARY_PATH"] = (
                    self.mpihome + "lib:" + self.mpihome + "lib/openmpi"
                )
            if "LD_RUN_PATH" in os.environ:
                if self.mpihome + "lib/openmpi" not in os.environ["LD_RUN_PATH"]:
                    os.environ["LD_RUN_PATH"] = (
                        self.mpihome + "lib/openmpi:" + os.environ["LD_RUN_PATH"]
                    )
            else:
    ...
    def tidy_wfn_jobs_folder(self):
        ...
        if os.path.exists(wfn_job_dir):
            files = list(
                file for file in os.listdir(wfn_job_dir) if "backup_" not in file
            )
            if len(files) != 0:
                os.mkdir(backup)
            for f in files:
                    ...
                    if run > 0:
                        if self.wfn_code == "Tonto":
                            ...
                            if ".gbw" not in f:
                                ...
                                shutil.move(
                                    os.path.join(wfn_job_dir, f),
                                    os.path.join(wfn_job_dir, self.name + "2.gbw"),
                                )
    ...
    def launch(self):
        ...
        if "Please S" in wfn_code and update == True:
            olx.Alert(
                "No tsc generator selected",
                """Error: No generator for tsc files selected.
Please select one of the generators from the drop-down menu.""",
                "O",
                False,
            )
        ...
        if (
            ...
            if (
                heavy
                and ("x2c" not in basis)
                and ("jorge" not in basis)
                and ("ECP" not in basis)
                and ("STO" not in basis)
                and ("3-21" not in basis)
            ):
            ...
            if (ne % 2 == 0) and (mult % 2 == 0):
                print(
                    "Error! Multiplicity and number of electrons is even. This is impossible!\n"
                )
                OV.SetVar("NoSpherA2-Error", "Multiplicity")
                return False
            elif (ne % 2 == 1) and (mult % 2 == 1):
                print(
                    "Error! Multiplicity and number of electrons is uneven. This is impossible!\n"
                )
                OV.SetVar("NoSpherA2-Error", "Multiplicity")
                return False
            if (wfn_code == "ELMOdb") and (mult > 1):
                print(
                    "Error! Multiplicity is not 1. This is currently not supported in ELMOdb. Consider using QM/ELMO instead!\n"
                )
                OV.SetVar("NoSpherA2-Error", "Multiplicity")
                return False
        ...
        if OV.GetParam("snum.NoSpherA2.no_backup") == False:
            ...
            for file in os.listdir(olx.FilePath()):
                ...
                if file.endswith(".tscb"):
                    tsc_exists = True
                    f_time = os.path.getmtime(file)
            if tsc_exists and not any(
                x in wfn_code for x in [".wfn", ".wfx", ".molden", ".gbw", ".fchk"]
            ):
                import datetime
                ...
                timestamp_dir = os.path.join(
                    ...
                    olx.FileName()
                    + "_"
                    + datetime.datetime.fromtimestamp(f_time).strftime(
                        "%Y-%m-%d_%H-%M-%S"
                    ),
                )
                ...
                for file in os.listdir("."):
                    if any(file.endswith(x) for x in backup_endings):
                        shutil.move(
                            os.path.join(olx.FilePath(), file),
                            os.path.join(timestamp_dir, file),
                        )
        ...
        if nr_parts > 1:
            ...
            shutil.move(
                self.name + ".cif_NoSpherA2",
                os.path.join(self.jobs_dir, self.name + ".cif"),
            )
            if wfn_code == "fragHAR":
            ...
            for i in range(nr_parts):
                ...
                if wfn_code.lower().endswith(".fchk"):
                    raise NameError(
                        "Disorder is not possible with precalculated fchks!"
                    )
                try:
                ...
                out_cif = open(
                    os.path.join(wfn_job_dir, "%s.cif" % (OV.ModelSrc())), "w"
                )
                with open(
                    os.path.join(self.jobs_dir, "%s.cif" % (OV.ModelSrc())), "r"
                ) as incif:
                    for line in incif:
                ...
                if wfn_code == "discambMATTS":
                    # DISCMAB is used
                    discamb(
                        os.path.join(OV.FilePath(), wfn_job_dir),
                        self.name,
                        self.discamb_exe,
                    )
                    shutil.copy(
                        os.path.join(wfn_job_dir, self.name + ".tsc"),
                        self.name + "_part_" + str(parts[i]) + ".tsc",
                    )
                    shutil.copy(
                        os.path.join(wfn_job_dir, "discambMATTS2tsc.log"),
                        os.path.join(self.jobs_dir, "discamb2tsc.log"),
                    )
                    need_to_combine = True
                elif wfn_code == "Hybrid":
                    # We are in Hybrid mode
                    hybrid_part_wfn_code = OV.GetParam(
                        "snum.NoSpherA2.Hybrid.software_Part%d" % (parts[i])
                    )
                    if hybrid_part_wfn_code == "discambMATTS":
                        ...
                        shutil.move(
                            "%s_part_%s.xyz" % (self.name, parts[i]),
                            os.path.join(wfn_job_dir, "%s.xyz" % (self.name)),
                        )
                        try:
                    ...
                    if wfn_code != "Tonto":
                        ...
                        if wfn_code == "ELMOdb":
                            ...
                            if os.path.exists(os.path.join(OV.FilePath(), pdb_name)):
                                shutil.copy(
                                    os.path.join(OV.FilePath(), pdb_name),
                                    os.path.join(wfn_job_dir, self.name + ".pdb"),
                                )
                            else:
                                OV.SetVar("NoSpherA2-Error", "ELMOdb")
                                if mutation == True:
                                    raise NameError(
                                        "No pdb_name file available for mutation!"
                                    )
                                else:
                                    raise NameError(
                                        "No pdb file available! Make sure the name of the pdb file is the same as the name of your ins file!"
                                    )
                        OV.SetParam("snum.NoSpherA2.fchk_file", self.name + ".fchk")
                        try:
                            self.wfn(
                                folder=wfn_job_dir, xyz=False, part=parts[i]
                            )  # Produces Fchk file in all cases that are not fchk or tonto directly
                        except NameError as error:
                            print("Aborted due to: ", error)
                            OV.SetVar("NoSpherA2-Error", error)
                            return False
                    if experimental_SF == False or wfn_code == "Tonto":
                        ...
                        if (
                            "Error in"
                            in open(
                                os.path.join(job.full_dir, job.name + ".err")
                            ).read()
                        ):
                        ...
                        if OV.HasGUI():
                            olx.html.Update()
                        shutil.copy(
                            os.path.join(job.full_dir, self.name + ".tsc"),
                            self.name + "_part_" + str(parts[i]) + ".tsc",
                        )
                    elif wfn_code == "Thakkar IAM" or wfn_code == "SALTED":
                        wfn_fn = os.path.join(
                            OV.FilePath(), wfn_job_dir, self.name + ".xyz"
                        )
                    else:
                        wfn_fn = None
                        path_base = os.path.join(OV.FilePath(), wfn_job_dir, self.name)
                        if os.path.exists(path_base + ".wfx"):
                            if (
                                wfn_fn == None
                                or wfn_fn.endswith(".wfn")
                                or wfn_fn.endswith(".fchk")
                            ):
            ...
            if "discambMATTS" in wfn_code:
                ...
                olx.File(cif)
                discamb(
                    os.path.join(OV.FilePath(), self.jobs_dir),
                    self.name,
                    self.discamb_exe,
                )
                shutil.copy(
                    os.path.join(OV.FilePath(), self.jobs_dir, self.name + ".tsc"),
                    self.name + ".tsc",
                )
                OV.SetParam("snum.NoSpherA2.file", self.name + ".tsc")
            else:
                if (
                    wfn_code.lower().endswith(".wfn")
                    or wfn_code.lower().endswith(".wfx")
                    or wfn_code.lower().endswith(".molden")
                    or wfn_code.lower().endswith(".gbw")
                ):
                    pass
                elif wfn_code == "Tonto":
                    job = Job(self, self.name)
                    ...
                    if experimental_SF == False:
                        shutil.copy(
                            os.path.join(job.full_dir, job.name + ".tsc"),
                            job.name + ".tsc",
                        )
                    ...
                    if wfn_code == "ELMOdb":
                        # copy the pdb
                        if os.path.exists(
                            os.path.join(OV.FilePath(), self.name + ".pdb")
                        ):
                            shutil.copy(
                                os.path.join(OV.FilePath(), self.name + ".pdb"),
                                os.path.join(self.jobs_dir, self.name + ".pdb"),
                            )
                        else:
                            OV.SetVar("NoSpherA2-Error", "ELMOdb")
                            raise NameError(
                ...
                if experimental_SF == True:
                    if wfn_code != "fragHAR" and wfn_code != "SALTED":
                        ...
                        if (
                            ...
                            if wfn_code == "Thakkar IAM":
                                ...
                                os.path.exists(path_base + x) for x in endings
                            ):
                    ...
                    if (
                        "Error in"
                        in open(os.path.join(job.full_dir, job.name + ".err")).read()
                    ):
                        success = False
                        with open(
                            os.path.join(job.full_dir, job.name + ".err")
                    ...
                    if OV.HasGUI():
                        olx.html.Update()
                    shutil.copy(
                        os.path.join(job.full_dir, job.name + ".tsc"), job.name + ".tsc"
                    )
                    OV.SetParam("snum.NoSpherA2.file", job.name + ".tsc")
        # add_info_to_tsc()
    ...
    def wfn(self, folder="", xyz=True, part=0):
        ...
        if software == "fragHAR":
            ...
            relativistc = OV.GetParam(
                "snum.NoSpherA2.Hybrid.Relativistic_Part%d" % part
            )
            ...
            strategy = OV.GetParam(
                "snum.NoSpherA2.Hybrid.ORCA_SCF_Strategy_Part%d" % part
            )
    ...
    def setup_discamb(self):
        self.discamb_exe = self.setup_software(
            OV.GetParam("user.NoSpherA2.discamb_exe"),
            OV.GetParam("user.NoSpherA2.discamb_exe"),
        )
        if not os.path.exists(self.discamb_exe):
            self.discamb_exe = self.setup_software(
                "discambMATTS2", "discambMATTS2tsc", True
            )
    ...
    def getBasisListStr(self):
        ...
        if (
            ...
            if max_Z <= 86 and max_Z > 36:
                return (
                    final_string
                    + ";ECP-def2-SVP;ECP-def2-TZVP;ECP-def2-TZVPP;ECP-def2-QZVP;ECP-def2-QZVPP"
                )
        return final_string
...
def discamb(folder, name, discamb_exe):
    ...
    if not os.path.exists(hkl_file):
        ...
        with open(hkl_file, "w") as out:
            ...
            f_sq_obs = f_sq_obs.complete_array(
                d_min_tolerance=0.01,
                d_min=f_sq_obs.d_max_min()[1],
                d_max=f_sq_obs.d_max_min()[0],
                new_data_value=-1,
                new_sigmas_value=-1,
            )
    ...
    p = subprocess.Popen([pyl, os.path.join(p_path, "discamb-launch.py")])
...
class Job(object):
    ...
    def launch(self, wfn_dir=""):
        ...
        if fchk_source == "Tonto":
            ...
            args = [
                self.name + "_tonto.cif",
                "-basis-dir",
                self.parent.basis_dir,
                "-shelx-f2",
                self.name + ".hkl",
                "-basis",
                OV.GetParam("snum.NoSpherA2.basis_name"),
                "-cluster-radius",
                str(OV.GetParam("snum.NoSpherA2.cluster_radius")),
                "-dtol",
                OV.GetParam("snum.NoSpherA2.DIIS"),
            ]
        ...
        if OV.GetParam("snum.NoSpherA2.ncpus") != "1":
            args = [
                self.parent.mpiexec,
                "-np",
                OV.GetParam("snum.NoSpherA2.ncpus"),
                self.parent.mpi_har,
            ] + args
        else:
        ...
        p = subprocess.Popen([pyl, os.path.join(p_path, "HARt-launch.py")])
        tries = 0
        while tries < 20:
...
def add_info_to_tsc():
    tsc_fn = os.path.join(
        OV.GetParam("snum.NoSpherA2.dir"), OV.GetParam("snum.NoSpherA2.file")
    )
    ...
    if ubuntu_exe != None and os.path.exists(ubuntu_exe):
        ...
        try:
            child = subprocess.Popen(
                [ubuntu_exe, "run", "python -c 'import pyscf' && echo $?"],
        ...
        if loud == True:
            print(
                "To use pySCF please install pySCF in your python environment:\nsudo apt update\nsudo apt install python python-numpy python-scipy python-h5py python-pip\nsudo -H pip install pyscf"
            )
        return False
    else:
        if sys.platform[:3] == "win":
            if loud == True:
                print(
                    "To use pySCF please install ubuntu and Windows Subsystem for Linux (WSL) and afterwords run inside the WSL temrinal:\nsudo apt update\nsudo apt install python python-numpy python-scipy python-h5py python-pip\nsudo -H pip install pyscf"
                )
        else:
            if loud == True:
...
def change_tsc_generator(input):
    if input == "Get ORCA":
        ...
        if result == False:
            selection = olx.Alert(
                "No working pySCF installation found!",
                """Error: No working pySCF installation found!.
Do you want to have a guide how to install pySCF?""",
                "YN",
                False,
            )
            if selection == "Y":
                ...
                if win:
                    ...
                    if wsl == None or os.path.exists(wsl) == False:
                        olx.Alert(
                            "Could not find Windows Subsystem for Linux",
...
step of installation will be shown if the installation was succesfull""",
                            "O",
                            False,
                        )
                        return
...
Please do this now and klick 'Ok' once everything is done!""",
                            "O",
                            False,
                        )
                        while not cont:
                            ...
                            if os.path.exists(ubuntu_exe) == False:
                                ...
                                if tries == 4:
                                    print(
                                        "Something seems wrong, aborting installation guide after 3 unsuccesfull attempts!"
                                    )
                                    return
                                olx.Alert(
                                    "Could not find Ubuntu",
                                    """pySCF requires Ubuntu to be installed on your system.
It is reletively easy to do that.
You can go to the Microsoft store and install it by searching for 'ubuntu' in clicking the install button.
Once it is downloaded click the 'Launch' button and set up a username and password.
                    ...
                    if test == False:
                        while not cont:
                            ...
                            if tries == 4:
                                ...
                                return
                            olx.Alert(
                                "Please install Python and pySCF in Ubuntu",
...
Please do this now and klick 'Ok' once everything is done!""",
          ...
          Do you want to restart Olex2 now to load the new settings?""",
                                    "YN",
                                    False,
                                )
                                if selection == "Y":
...
Once you click 'Ok' Olex2 will do so automatically!""",
                            "O",
                            False,
                        )
                        print("PySCF installed sucessfully!")
...
python -c 'import pyscf'
If that does not throw an error message you were succesfull.""",
                        "O",
                        False,
                    )
