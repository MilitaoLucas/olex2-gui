...
def load_object(name: str, rel_directory: str = "wfn_job_data", int_keys: bool = False):
	name = f"{name}.json" if not name.endswith(".json") else name
	path = os.path.join(rel_directory, name)
	with open(path, "r", encoding="utf-8") as f:
		raw = json.load(f)
	if int_keys:
		data: Dict[int, Any] = {int(k): v for k, v in raw.items()}
		return data
	return raw
...
class wfn_Job(object):
	...
	def __init__(self, parent, name, _dir, software=None):
		self.ELEMENTS = load_object("periodic_table", int_keys=True)
	...
	def write_orca_crystal_input(self,xyz):
		known_charges = load_object("known_charges")
  ...
  mf.kernel()"""
	...
	def write_pyscf_script(self,xyz,basis_name=None,method=None,relativistic=None,charge=None,mult=None,damp=None,part=None):
		solv_epsilon = load_object("solv_epsilon")
  ...
  fout.write(' THE SCF ENERGY =%20.12f THE VIRIAL(-V/T)=   0.00000000\\n'%tot_ener)"""
		...
		if not pbc:
			...
			if method != "HF":
				rest += "mf.grids.radi_method = dft.gauss_chebyshev\n"
			rest += "mf.grids.level = "+str(grid)+"\n"
			rest += f"""mf.with_df.auxbasis = df.aug_etb(mol, beta={1.75 if grid >= 3 else 2.0})
mf.diis_space = 19
	...
	def run(self,part=0,software=None,basis_name=None, copy = True):
		...
		if software != "pySCF":
			...
			if software == "ORCA 5.0" or software == "fragHAR" or software == "ORCA 6.0" or software == "ORCA 6.1":
				...
				elif self.parent.ubuntu_exe is None:
