...
class comment(html_tag):
  ...
  COMMENT_KEYWORDS = ["#include", "#includeif", "#ignoreif"] # Special comment keywords added for Olex2 directives
  ...
  def _render(self, sb, indent_level=1, indent_str='  ', pretty=True, xhtml=False):
    ...
    if self[0].strip().split(" ")[0] in self.COMMENT_KEYWORDS and len(sb) > 0:
        if sb[-1] == indent_str * indent_level:
            sb[-1] = ""
