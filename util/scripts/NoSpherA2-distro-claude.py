...
the NoSpherA2 build already ships; the Linux asset is static. The macOS
asset may be dynamically linked - check it with `otool -L ptb_macos` and put
any non-system libraries into --ptb-libs-dir-mac (they land at the zip root).

Usage:
    python NoSpherA2-distro-claude.py                       # ptb from the latest release
    python NoSpherA2-distro-claude.py --ptb-tag v3.9-NoSpherA2_with_Lanthanides
    python NoSpherA2-distro-claude.py --ptb-dir ./my-ptb    # local ptb.exe / ptb_linux_static / ptb_macos
    python NoSpherA2-distro-claude.py --no-ptb --occ-share-dir D:/git/NoSpherA2/occ/share
    python NoSpherA2-distro-claude.py --include-tests
    python NoSpherA2-distro-claude.py --artifacts-dir ./artifacts --extras-dir . --out-dir ./dist
...
OCC_SHARE_DEFAULT_SOURCE = r"D:\git\NoSpherA2\occ\share"
        ...
        if ptb_dir and zip_name in PTB_ASSETS:
            ...
            if ptb_libs_dir:
                if os.path.isdir(ptb_libs_dir):
                    n = add_dir_to_zip(zf, ptb_libs_dir)
                ...
                print("  Note: ptb_macos may be dynamically linked - check `otool -L` and pass "
                      "--ptb-libs-dir-mac if it needs non-system libraries")
...
def main():
    ...
    for zip_name, (platform_rel, extra_name) in PLATFORMS.items():
        ...
        build_zip(out_path, basis_sets_dir, occ_share_dir, args.occ_share_full, platform_dir, extra_dir,
